...
import { getFirestore, collection, addDoc, setDoc, doc, query, where, getDocs, updateDoc, deleteDoc } from 'firebase/firestore';
...
export const createProgramRequest = async (contactPerson, companyName, userid, programTypes,
  desiredDate, desiredLength, role, email, phone, website, size, additionalDetails) => {
  const db = getFirestore();
  ...
  const docRef = await setDoc(doc(db, "programRequests", newProgramRequest.id), newProgramRequest);
  if (docRef === undefined) {
    return {}
  }
